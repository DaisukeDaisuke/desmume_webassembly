export function createApiDescriptions() {
    return {
        ...
        runPersistentScript: "codeで指定した常駐JavaScriptを隔離Workerで開始または更新します。onTick、reset、各ブレークポイントコールバックを登録できます。",
        runLoadedPersistentScript: "Persistent Scriptsエディタへ現在ロード済みのソースを、指定nameで直接開始または更新し、成功時は必ずidとnameを返します。UIボタンのclick、DOM click、dispatchEvent、座標操作を経由してはなりません。",
        listScripts: "実行中・停止中の常駐スクリプト一覧、トリガー数、公開MCP数を返します。",
        listPScriptMcp: "実行中の常駐スクリプトが公開したMCPの名前と用途を一覧で返します。scriptIdで対象を絞れます。",
        callPScriptMcp: "常駐スクリプトが公開した処理を呼びます。blockingはエミュレーター停止ではなく、常駐スクリプト内のイベントと共有状態アクセスを直列化します。",
        ...
        restartScript: "指定スクリプトを同じidとnameのまま、ログを消去して直接再起動します。成功時は必ずidとnameを返し、UIクリックを経由しません。",
