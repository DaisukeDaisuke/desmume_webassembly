export function createApiDescriptions() {
    return {
        status: "現在の停止状態、State適用serial、file transaction、フレーム、速度、描画/音声/デバッグ設定、PC/CPSRを返します。",
        getOperation: "現在のlong-running operationと開始時刻、経過時間、取消状態を返します。",
        cancelOperation: "現在のlong-running operationだけを取消し、cleanup完了まで待ちます。",
        ...
        listAnalysisBaselines: "解析baselineのmetadata一覧を返します。",
        deleteAnalysisBaseline: "解析baselineのmetadataと内部Stateを削除します。",
        listStateSlots: "通常State slot一覧を返します。",
        deleteStateSlot: "指定した通常State slotを削除します。",
        listSaveSlots: "カートリッジSave slot一覧を返します。",
        deleteSaveSlot: "指定したカートリッジSave slotを削除します。",
        ...
        runInputHold: "エミュレーター動作中に指定ボタンを一定時間維持します。停止中はerrorになり、手動pauseだけresume:trueを指定できます。",
        runInputTap: "エミュレーター動作中に指定ボタンをms単位で一定回数連打します。停止中はerrorになり、手動pauseだけresume:trueを指定できます。",
        runTouchHold: "エミュレーター動作中に下画面の座標を一定時間押し続けます。停止中はerrorになり、手動pauseだけresume:trueを指定できます。",
        ...
        clearBreakpoints: "既定ではuser所有だけを一括削除し、script/operation所有を保持します。",
        ...
        stepOver: "ARM9は利用可能ならactive Stack Trace laneをJS stepし、同じlane・開始depthで逐次PCへ戻るまで進めます。Stack TraceがOFFなら有効化してdepthから開始し、すでにONなのにactive laneが無い場合は既存のnative step overへフォールバックします。ARM7はnative step overを使います。",
        ...
        nextCallThisDepth: "ARM9のactive Stack Trace lane内の次の直接関数呼び出しまで進めます。Stack TraceがOFFなら有効化してdepthから開始し、すでにONなのにactive laneが無い場合はdepthベース経路へフォールバックします。別laneの深度変化は無視し、開始laneが浅くなるか消えた場合はroot境界で停止します。ARM7では利用できません。",
        ...
        runPersistentScript: "codeで指定した常駐JavaScriptを隔離Workerで開始または更新します。nameは省略でき、API名、@script-idコメント、runtime returnのfallbackId、script-<id>仮名の順で識別します。",
        runLoadedPersistentScript: "Persistent Scriptsエディタのソースを直接開始します。nameは省略可能で、waitForRegistrationは既定trueです。結果にはid、確定名または仮名、topLevelRunning、registrationComplete、短いMCP一覧が含まれます。UIボタンのclick、DOM click、dispatchEvent、座標操作を経由してはなりません。",
        listScripts: "実行中・停止中の常駐スクリプト一覧、識別元、top-level状態、登録完了状態、トリガー数、公開MCP数を返します。登録待ちはmcpCount:nullです。",
        listPScriptMcp: "実行中の常駐スクリプトが公開したMCPの名前と用途を一覧で返します。scriptIdで対象を絞れます。",
        callPScriptMcp: "常駐スクリプトが公開した処理を呼びます。scriptId/id/scriptNameは省略でき、MCP名が一意なら自動解決します。timeoutMs既定値は60000msで、timeoutエラーは変更キーを示します。blockingはエミュレーター停止ではなく共有状態アクセスの直列化です。",
        ...
        restartScript: "指定スクリプトを同じnumeric idで直接再起動します。API名は維持し、コメントIDまたはruntime fallbackは同じソースから再確定します。成功時は同じsummary形式を返し、UIクリックを経由しません。",
        getScript: "指定スクリプトのJavaScript本文を上限付きで返します。truncatedとoriginalCharsで切り詰めを確認できます。",
        listScriptPrint: "常駐スクリプトのコンソール出力を行番号付きで最大max件返します。startLineで取得開始行を指定できます。",
        clearScriptPrint: "指定スクリプトまたは全スクリプトのコンソール出力を消去し、次の行番号を返します。",
