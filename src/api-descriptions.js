export function createApiDescriptions() {
    return {
        ...
        stepOver: "ARM9は記録済みactive Stack Trace laneをJS stepし、同じlane・開始depthで逐次PCへ戻るまで進めます。同じlaneが浅くなるか消えた場合はroot境界で停止します。ARM7はStack Trace非対応のためnative step overを使います。",
        ...
        nextCallThisDepth: "ARM9の記録済みactive Stack Trace lane内の次の直接関数呼び出しまで進めます。別laneの深度変化は無視し、開始laneが浅くなるか消えた場合はroot境界で停止します。ARM7では利用できません。",
