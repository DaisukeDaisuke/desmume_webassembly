...
- `stackTrace`: Returns the recorded call stack plus stack words near SP for `{ "cpu": "arm9", "words": number }`. Call-stack `caller` addresses are call-site instruction addresses (`returnAddress - 4`); the raw LR is shown separately as `return`.
- `callStack`: Returns the recorded call stack as structured JSON with `frames` ordered newest-first. In each frame, `caller` is the call-site instruction address (`returnAddress - 4`), `returnAddress` is the raw LR/return address, and `ageLabel` is `newest` for the first frame or `↑+Nd` for older frames below it. Besides `frames`, the result includes a `controlFlow` list for traced ARM9 PC-write/return events such as `BX`, `MOV PC`, `MOVS PC`, `SUBS PC`, `LDM ... {PC}`, and `LDM ... {PC}^`; each event includes `pc`, `target`, `expected`, `sp`, `cpsr`, `kindName`, and `mismatch`.
