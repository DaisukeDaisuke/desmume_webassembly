...
- `importSaveFile`: Opens the file picker for a `.sav` file and imports it into the cartridge save buffer.
- `exportSaveFile`: Downloads the current cartridge save as `desmume-save.sav`.
...
- `loadState`: Loads the active in-memory state or a named browser storage slot. Loading while paused keeps the emulator paused.
- `importStateFile`: Opens a file picker and loads an external state file into the emulator.
...
- `setAudio`: Sets `{ "enabled": boolean, "volume": 0..1 }`. Disabling audio stops output while emulation continues.
...
- `disassemble`: Dumps address/opcode rows with `{ "cpu": "arm9", "address": number|string, "count": number, "mode": "auto"|"arm"|"thumb" }`.
...
- `step`: Runs `{ "count": N }` CPU steps through the native debugger entry point.
- `stepOver`: Runs one native step-over entry point.
