...
- `callStack`: Returns the recorded call stack as structured JSON with `frames` ordered newest-first for the active stack lane. In each frame, `caller` is the call-site instruction address (`returnAddress - 4`), `returnAddress` is the raw LR/return address, and `ageLabel` is `newest` for the first frame or `↑+Nd` for older frames below it. `limit` defaults to 128 and caps returned frames at 1024. When SP moves by a large switch threshold, frames are separated into `stacks`; each stack has `id`, `sp`, `nowPc`, `depth`, `active`, and its own limited newest-first `frames`. Non-matching PC writes select the lane nearest the current SP and update `nowPc` without discarding recorded frames. Besides `frames`, the result includes a `controlFlow` list for traced ARM9 PC-write/return events such as `BX`, `BLX reg`, `MOV PC`, `MOVS PC`, `SUBS PC`, `LDM ... {PC}`, and `LDM ... {PC}^`; each event includes `pc`, `target`, `expected`, `sp`, `cpsr`, `kindName`, and `mismatch`.
