...
The current public registry exposes 144 command names. Compatibility aliases are included in this count. `window.DesmumeMCP.list()` is the runtime source of truth; this inventory is the reviewable document counterpart.
- System and context: `status`, `getOperation`, `cancelOperation`, `snapshotContext`, `saveAnalysisBaseline`, `restoreAnalysisBaseline`, `listAnalysisBaselines`, `deleteAnalysisBaseline`, `pause`, `resume`, `continue`, `reset`, `reloadRom`, `setSpeed`, `setRenderEnabled`, `setAudio`, `setScale`, `setRotation`, `setAutoUpdate`, `setUiInteractionLock`, `setFeatureSet`.
...
- Disassembly and debugger control: `disassemble`, `disassembleBytes`, `setBreakpoint`, `setSpecialBreakpoint`, `listBreakpoints`, `removeBreakpoint`, `clearBreakpoints`, `clearBreakStatus`, `step`, `smartStep`, `stepOver`, `stepNextBranchOrReturn`, `nextBranchOrReturn`, `trueNextBranch`, `nextTrueBranch`, `setStackTraceMode`, `setStackTracePrivilegeCheck`, `stackTrace`, `callStack`, `listOtherCoroutines`, `getOtherCoroutines`, `copyCallStackMarkdown`, `copyCallStackCsv`, `runUntilReturn`, `runUntilNextCall`, `nextCallThisDepth`, `returnToPop`, `nextFunctionEnter`, `nextCall`, `nextFunctionCall`.
...
- `setUiInteractionLock`: Adds or removes an owner-scoped human-UI lock with `{ "owner": "micro-macro:turn-flow", "locked": true|false }`. While at least one owner remains locked, the page visually disables buttons, file-button labels, form controls, on-screen DS buttons, and touch-screen pointer input, and capture-blocks trusted pointer/mouse/touch/keyboard/wheel events before normal UI handlers run. It does not pause or resume the emulator and does not block `DesmumeMCP.call()`, WebMCP, persistent scripts, Worker RPC, or programmatic file-input changes. Unlocking removes only the caller's owner; the UI stays locked while any other owner remains. The result is `{ locked, ownerCount, owners }`.
- `setInput`: Presses or releases DS buttons using `{ "button": "A|B|X|Y|L|R|Start|Select|Up|Down|Left|Right", "pressed": boolean }`. The shared key state drives both emulation and the on-screen key feedback. Paused input returns `INPUT_UNAVAILABLE` with a bounded pause summary. For manual pause only, pass `"resume":true` to await resume before mutating input; breakpoint pauses remain errors.
- `runInputHold`: Holds one or more buttons for a timed interval using `{ "button": "A" }` or `{ "buttons": ["Up","A"] }`, with optional `{ "durationMs": 500, "waitBeforeMs": 0, "waitAfterMs": 0, "timeoutMs": 2000, "resume": true }`. `resume:true` applies only to manual pause.
- `runInputTap`: Repeats one or more buttons with `{ "button": "A" }` or `{ "buttons": ["Left","B"] }`, with optional `{ "repeat": 5, "holdMs": 40, "gapMs": 50, "waitBeforeMs": 0, "waitAfterMs": 0, "timeoutMs": 2000, "resume": true }`. `resume:true` applies only to manual pause.
...
- `smartStep`: Looks at the current disassembly line and chooses a safer single-step mode automatically. Ordinary instructions use `step`; `bx*`, `bl*`, and `blx*` use the legacy native step-over implementation directly. It does not call the public trace-based ARM9 `stepOver`. Plain `b*` and `add/sub ... pc` stay as one-instruction steps.
- `stepOver`: Accepts `{ "cpu": "arm9"|"arm7", "timeoutMs": number, "maxSteps": number }`. ARM9 uses the recorded Stack Trace lane when available. If Stack Trace is off, it is enabled and the first step-over bootstraps from the current native depth instead of failing; if Stack Trace was already enabled but has no recorded active lane, that invocation falls back to the existing native step-over implementation instead of failing. The lane-aware path JS-steps until PC reaches the sequential ARM/Thumb instruction address while execution is back on the same starting lane and depth, or stops at `root` if that lane becomes shallower or disappears. A matching address reached recursively at a deeper depth or on another lane is not success. Each JS step reapplies Memory Freeze values. Breakpoints and native faults still interrupt safely. Timeout or `maxSteps` returns `complete:false`, `limitReached:true`, and `stop:"timeout"|"maxSteps"` on the lane-aware trace path instead of throwing. ARM7 uses the same existing native step-over implementation.
...
- `callStack`: Returns the active lane rows shown by the UI in newest-first order, including UI-visible `pc-write` rows so age labels remain contiguous. Internal fields such as `synthetic`, `expected`, `target`, `kind`, `mode*`, and `controlFlow` are withheld by default; active-lane frames are returned only once in the top-level `frames`, while `stacks` contains lane summaries without nested frame copies. Real rows include caller/callee addresses, return address, SP, CPSR, CPU mode, ISA, and 1-3 disassembly lines at each caller/callee point. `synchronized:false` means a State load restored CPU/RAM without matching WASM trace metadata, so active-depth operations such as `runUntilReturn` are rejected until trace is explicitly restarted. Raw diagnostic data is available only with `{ "raw": true }`; it is intended only for debugging the call-stack implementation and should not be used for reverse engineering or normal analysis.
...
- `runUntilReturn`: Steps until the recorded call stack depth drops below the current depth. Pass `{ "timeoutMs": number, "maxSteps": number }`. Timeout or the step limit returns an incomplete stopped result. If the current instruction address itself has an exec breakpoint, only that one is suspended for the single trace-step and then restored. The result includes `pcBefore`, `pcAfter`, minimal `status`, hex `registers`, and near-PC `disassembly`.
- `runUntilNextCall`: Steps until the next function-entry hook is recorded. Pass `{ "timeoutMs": number, "maxSteps": number }`. Timeout or the step limit returns an incomplete stopped result. If the current instruction address itself has an exec breakpoint, only that one is suspended for the single trace-step and then restored. The result includes `pcBefore`, `pcAfter`, minimal `status`, hex `registers`, and near-PC `disassembly`.
- `nextCallThisDepth`: ARM9-only lane-aware trace stepper. It binds the starting `activeStackId` and call depth when a recorded lane exists, ignores temporary entry-hook frames that retain the starting frame's return address, ignores depth changes while another lane is active, and stops only when the instruction executed at that starting depth creates a call frame. Calls made after entering a child frame do not satisfy this condition. If Stack Trace is off, it is enabled and the first invocation bootstraps from the current native depth. If Stack Trace was already enabled but has no recorded active lane, the command falls back to the existing depth-based next-call path instead of failing; successful fallback completion reports `stop:"call"` just like the lane-aware path. If the starting lane returns or disappears first, it stops with `complete:false` and `stop:"root"`. Timeout and `maxSteps` return an incomplete stopped result.
...
- `runTouchHold`: Holds the lower touch screen at a DS coordinate using `{ "x": 128, "y": 96, "durationMs": 300, "waitBeforeMs": 0, "waitAfterMs": 0, "timeoutMs": 2000, "resume": true }`. Paused input is rejected; `resume:true` applies only to manual pause.
...
- `batch`: Runs multiple WebMCP commands sequentially. Pass `{ "commands": [{ "command": "status", "params": {} }] }`; the result contains one entry per command. `takeScreenshot({includeDataUrl:false})` contributes metadata only and does not include an undefined or image-sized `dataUrl` field.
...
`waitForBreak`, `runUntil`, `runUntilMemoryRead`, `runUntilMemoryWrite`, `runInputSequence`, `recordInput`, `replayInput`, `waitForPause`, `waitForScreenChange`, `waitForFrameMatch`, and `waitForScreenStable` share one operation lock. A second long-running operation returns `ok:false` with `error.code="BUSY"`. `waitForStateLoad` and `waitForFileTransaction` are independent serial watchers so the file load they observe cannot cancel them. `getOperation`, `cancelOperation`, `getInputState`, and `releaseInput` remain available for recovery.
Waits, recording, and replay require `timeoutMs` in the range 1 through 600000 unless the individual command documents an existing default. Timeout is a normal result with `error.code="TIMEOUT"`, not a rejected Promise. Cleanup releases timers, listeners, operation breakpoint owners, DS buttons, and touch input.
...
```
Sequences require a running emulator. Pass `{ "resume": true }` only to await release of a manual pause before the first input; execution, memory, and special breakpoint pauses still return `INPUT_UNAVAILABLE`.
...
`runInputSequence` executes a caller-authored compact tuple sequence. `recordInput` instead records the complete input snapshots actually produced by keyboard, on-screen controls, touch, and API input at completed-frame offsets. `recordInput` requires `id`, `timeoutMs`, and exactly one of `frames` or `durationMs`; `captureState:true` stores an associated State in IndexedDB.

`replayInput({id,timeoutMs,loadState:true})` loads the associated State inside the replay operation. `stateSlot` selects an existing State instead and cannot be combined with `loadState:true`. ROM identity and, by default, ARM9/ARM7 PC/CPSR are verified before the first input mutation. `verifyStart:false` returns `pcVerified:null` and `verificationSkipped:true`. Replay advances by completed frames, releases every button and touch on all exits, and returns paused by default; `pauseAfter:false` resumes and reports the resulting `paused`/`running` state. `listInputRecordings` returns metadata only; `deleteInputRecording` removes metadata, events, and associated State.
...
All three screen waits copy selected completed frames into a bounded ordered queue before asynchronous comparison. `waitForFrameMatch` compares against a saved snapshot. `waitForScreenStable` compares consecutive queued samples. Queue overflow is an explicit `RESOURCE_LIMIT` error; samples are never silently skipped while a comparison is in flight.

## Lazy runtime loading

Initial HTML loads only the minimized `public/app.js` bootstrap. It preserves the existing canvas and ROM picker and reports `emulatorLoaded:false`. `public/emulator.js` is not requested or evaluated until a ROM load command requests the runtime. Bootstrap status, operation recovery, input state, storage lists, `desmume.list`, and ROM-unloaded debugger calls do not import it. Concurrent ROM requests share one loader promise; a hard timeout invalidates the attempt, ignores its late completion, and permits a fresh retry. Later ROM reloads reuse the already-evaluated runtime bundle.

Debug memory reads and writes use native `MMU_AT_DEBUG` access and do not increment CPU memory-breakpoint hit counts. `runUntilMemoryRead` and `runUntilMemoryWrite` count only emulated CPU accesses and keep read/write semantics separate.
## Worker and bundled algorithm policy
Persistent/eval Worker sources live under `src/workers`, are embedded as strings in the production `public/app.js` bundle, and start from Blob URLs. Stop, startup timeout, crash, and restart terminate the Worker, settle pending RPC, and revoke its URL. A `callPScriptMcp` caller timeout does not terminate the Worker or handler. Their inner sandboxes cannot access the DOM, `window`, ROM/State bytes, frame pixels, network APIs, browser storage, raw messages, sub-Workers, or unapproved RPC. `localStorage`, `sessionStorage`, IndexedDB, and Cache API are explicitly shadowed even though normal browser Workers do not expose every one of those APIs.
...
`runPersistentScript` starts a locally isolated Worker and keeps it alive until `stopScript` is called. Calls are queued independently for each script context, so two scripts cannot corrupt one another's internal queue state. The default is blocking `{ "asyncMode": false }`. Enable `{ "asyncMode": true }` only for a non-blocking register-observation script. A script remains valid when it returns `[]`, returns `undefined`, contains comments only, performs side effects only, has no MCP or trigger, awaits forever, or intentionally runs forever. There is no timeout on the script body, top-level Promise, runtime identity calculation, or MCP publication.
- `runPersistentScript`: `{ "name": "watch-hp", "code": "...", "asyncMode": false }` starts source supplied directly in the API request. `name` is optional. Updating an explicit API name replaces its previous running copy, and identical running code in the same mode is not registered twice.
- `runLoadedPersistentScript`: `{ "name"?: "battle_observer_mcp", "asyncMode": false, "startupTimeoutMs": 10000, "waitForRegistration": true }` directly starts or updates the source currently loaded in the Persistent Scripts editor. It does not accept `code`. The UI omits `name` when the field is blank and writes the returned provisional or final name back to the field.
- Identity priority is explicit API `name`, source line comment `// @script-id battle_observer_mcp`, runtime return `fallbackId`, then deterministic `script-<numeric-id>`. Only a standalone line-comment directive is parsed statically; block comments and return contents are not identity syntax. `fallbackId` is read from the actual awaited top-level result and may be computed dynamically. A lower-priority disagreement records `identityConflict:true` and the declared value but does not stop or roll back the script. A comment/runtime rename collision leaves the new script on its deterministic provisional name, records collision diagnostics, and never stops the existing script.
- A top-level array remains the compatible MCP return format. The object format is `{ fallbackId?: string, mcps?: [...] }`. `mcps`, an empty array, `undefined`, and no return are all valid. MCP names and fallback IDs match `^[A-Za-z][A-Za-z0-9._-]{0,63}$`.
```js
const suffix = await calculateProfileId();
return {
  fallbackId: `battle_observer_${suffix}`,
  mcps: []
};
```
- `running:true` means the script record and Worker are active. `started:true` means the Worker exists, parsing and compilation succeeded, top-level execution began, and the numeric ID is fixed. `topLevelRunning:true` means the awaited top-level body has not settled. `registrationComplete:true` means the top-level Promise resolved, its return was validated, MCP publication completed, and runtime fallback identity was processed.
- `waitForRegistration` defaults to `true`. It waits without a script-body timeout until `registrationComplete:true`, then returns `mcpCount`, alphabetically sorted `mcpNames` limited to 16, and `mcpNamesTruncated`. A completed script with no MCP returns `mcpCount:0`. With `waitForRegistration:false`, the call returns at `started:true`; registration is still pending, `topLevelRunning:true`, and `mcpCount:null`. A caller or transport timeout does not terminate, delete, or roll back the script.
- `startupTimeoutMs` applies only to Worker creation, hardened parser startup, compilation handshake, and the initial `started` notification. It does not limit top-level computation, top-level return, runtime fallback calculation, event handlers, or MCP publication.
```js
...
// { ok:true, id:2, name:"battle_observer_mcp", identitySource:"runtime-return", running:true, started:true, topLevelRunning:false, registrationComplete:true, mcpCount:5, mcpNames:[...], source:"loaded-editor", ... }
```
...
- `restartScript` accepts `{ "id": 2 }`, `{ "scriptId": 2 }`, or `{ "name": "battle_observer_mcp" }`; omitting all selectors restarts the currently selected script for the UI button. It preserves the numeric ID and returns the same summary shape. An explicit API name remains fixed; comment and runtime identities are recalculated from the saved source. It accepts `waitForRegistration` and must be called directly; click/DOM/coordinate automation is prohibited.
```js
const restarted = await window.DesmumeMCP.call("restartScript", {
  id: 2,
  waitForRegistration: true
});
// { ok:true, id:2, name:"battle_observer_mcp", running:true, started:true, registrationComplete:true, mcpCount:5, reloaded:true, ... }
```
...
- `callPScriptMcp` always requires MCP `name` and boolean `blocking`. It accepts optional `scriptId`, alias `id`, or `scriptName`. With no selector, all running completed registrations are searched by MCP name; exactly one match is called, zero matches return a compact directory of script IDs, script names, and MCP names, and multiple matches return `SCRIPT_MCP_AMBIGUOUS` with only the matching scripts and require an explicit selector. It never chooses the first ambiguous match.
- `timeoutMs` defaults to 60000 and ranges from 1 through 600000. Timeout ends only the caller's wait. The handler continues, a blocking handler retains its FIFO position until settlement, its delayed result is discarded, and the script, publication, and state remain available. The timeout error explicitly says to increase `callPScriptMcp.timeoutMs` and includes `timeoutParameter:"timeoutMs"`.
- Persistent scripts cannot call `listPScriptMcp` or `callPScriptMcp` through their own `mcp.call`; isolated `desmume.eval` scripts can. In isolated scripts the handler result is `response.value`.
- `listScriptPrint: { max: 10, startLine?, id? | scriptId? | name? }` returns console lines with stable per-script `line` numbers plus available line bounds. Without `startLine` it preserves the latest-lines behavior. `clearScriptPrint: { id? | scriptId? | name? }` clears one console, or all consoles when no selector is supplied, and returns each cleared console's next line number.
...
```
When a persistent script exposes orchestration handlers, print human-readable output only when semantic state changes; do not repeat identical state every tick. Return named objects or arrays that both humans and callers can interpret:
```js
return [{
  name: "listActions",
  description: "Returns the currently available game UI actions.",
  handler: async (params, context) => {
    if (!context.blocking) throw new Error("listActions requires blocking:true");
    return [{ id: "menu.item", label: "Items", enabled: true }];
  }
}];
```
...
Inside a breakpoint callback, `await mcp.call("resume", {})` uses an event-release barrier. The callback first releases its right to hold the breakpoint pause, all callbacks for the same event must finish or release, the current execute breakpoint is stepped past when applicable, native pause is cleared, the emulation loop is woken, and only then does the `resume` Promise resolve. Its success reports `resumed:true`, `steppedPast:true`, `nativePaused:false`, `running:true`, and `paused:false`. It does not guarantee completion of the next frame.
Persistent scripts can call WebMCP commands through `mcp.call(command, params)` or its `webmcp.call` alias, subject to the async-mode restrictions above. Common emulator and debugger operations also have shortcuts such as `emu.status()`, `emu.step()`, `emu.smartStep()`, `emu.stepOver()`, `emu.stepNextBranchOrReturn()`, `emu.trueNextBranch()`, `emu.runUntilReturn()`, `emu.runUntilNextCall()`, `emu.nextCallThisDepth()`, `emu.stepFrames(params)`, and `emu.setInput(params)`. Blocking mode also permits `emu.pause()` and `emu.resume()`.
An allowlist miss rejects only that RPC with `COMMAND_NOT_ALLOWED`; malformed Worker protocol still stops the script. Breakpoints created directly by a persistent script are script-owned and are removed if that script stops. A callback may return JavaScript `undefined` implicitly. Exec/read/write callback payloads provide `address`, `pc`, `value`, `cpu`, and breakpoint fields; `frame` is provided by tick callbacks, not memory/exec callbacks.
ROM reload, ROM load, Save replacement, State load/import, analysis-baseline restore, and recent-file reload are rejected from inside a breakpoint callback because they require lifecycle handlers after the callback barrier. Invoke them after the callback returns or through the external API. This prevents a callback from waiting on its own `start` or `stateLoad` completion. Non-destructive commands remain available subject to normal mode restrictions.
Destructive or run-state-changing API success means the native mutation, JavaScript state update, input/operation cancellation, breakpoint reconciliation, file transaction, and required persistent-script lifecycle handlers have completed; it does not mean that work was merely queued. ROM and Save replacement remain natively paused while every running script processes `start`; State replacement remains paused while every running script processes `stateLoad`. Only then is the requested prior or explicit run state restored. Handler failure leaves the emulator paused and rejects the API. `resume` success guarantees native pause clearance and an immediate emulation-loop wake, but not completion of the next frame.
The helper `setCTableSeed` provides the JavaScript equivalent of the common `setCTable_jp.lua` pattern: it writes `0x4b539adb` at `0x02385f0c` and zero at the following word unless overridden.
