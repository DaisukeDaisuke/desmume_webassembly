...
- `saveAnalysisBaseline`: Saves a named browser state slot together with its pause/running and trace/`skipIrq` policy, ROM name, byte size, SHA-256, baseline state-format version, and bounded JSON returned by registered persistent-script baseline callbacks. Pass `{ "name": "before-menu" }`; an existing name is protected unless `{ "replace": true }` is explicit.
- `restoreAnalysisBaseline`: Verifies the current ROM and every saved persistent-script name/source SHA-256 before passing state bytes to native code. It then restores registered script JSON while paused, resets trace history, and restores the recorded pause/running and trace/`skipIrq` policy. Pass `{ "name": "before-menu" }`. The result includes the same compact fields as `snapshotContext`; this requires a loaded ROM because the state belongs to that ROM.
...
- `timeoutMs` defaults to 3000 and ranges from 1 through 600000. Timeout ends only the caller's wait. The handler continues, a blocking handler retains its FIFO position until settlement, its delayed result is discarded, and the script, publication, and state remain available.
...
Persistent scripts can make closure state part of an analysis baseline by registering one pair of built-in callbacks:
```js
let internalState = { phase: "menu", counter: 0 };

emu_registerbaseline(
  async ({ name, operation, blocking }) => internalState,
  async (savedJson, { name, operation, blocking }) => {
    internalState = savedJson;
  },
  10 // optional integer priority; default 0
);
// emu.registerBaseline(saveCallback, restoreCallback, priority) is the same registration function.
```
Both callbacks must be `async` functions. Missing/`null`/non-async callbacks leave the hook unregistered and return `false` without stopping the script; successful registration returns `true`. The optional priority is an integer from `-1000000` through `1000000`; higher priority scripts run later and therefore win when they request the same deferred restore policy. A save callback may return any bounded JSON-compatible value. `undefined` and `null` are both stored as JSON `null`; the restore callback return value is ignored. Each callback is always serialized with that script's event/MCP work and awaited as a blocking call. The 1500 ms limit starts when the callback begins; if it does not settle, the baseline operation fails with `TIMEOUT` and that Worker is stopped and discarded so a late result cannot commit state.

`saveAnalysisBaseline` first enters a global stopped file-transaction boundary, then captures registered state from the fixed set of currently running persistent scripts before serializing emulator State. Each entry stores the script name, priority, SHA-256 of the exact source, and callback JSON. `restoreAnalysisBaseline` commits the file transaction before validating an immutable script plan; the same script instance plan is used after native State load, while the emulator remains paused. The saved and current participant sets must match exactly; a missing, stopped, renamed, changed, duplicated, or unregistered script fails restoration with `STATE_INVALID`. Nested save/restore requests return `BUSY` immediately regardless of baseline name or call path. Restore-hook requests for `setSpeed`, `setRenderEnabled`, `setAudio`, `setScale`, and `setRotation` are deferred and applied FIFO only after every restore hook succeeds. A failure after native State load returns `STATE_PARTIALLY_RESTORED` with the emulator held paused.

Callback JSON is normalized independently in the sandbox Worker, supervisor Worker, and main thread. Only finite JSON primitives, dense arrays, and plain data objects are accepted; cycles, accessors, functions, symbols, binary objects, and exotic prototypes are rejected. Objects are rebuilt with a `null` prototype and data properties, so keys such as `__proto__`, `constructor`, and `prototype` remain ordinary JSON keys and cannot cause prototype pollution during save, storage, or restore. One script may store at most 256 KiB of normalized callback JSON, with a 2 MiB aggregate baseline limit. The bounded script JSON is stored in IndexedDB; localStorage contains only the small baseline manifest and integrity metadata. New manifests use metadata version 1 and persistent-script storage version 1; older inline callback JSON remains readable as storage version 0.
...
The current version also exposes `stateLoad` and `stateSave` notification events. They are independent of the blocking analysis-baseline callback pair above. Normal `mcp.call("loadState", ...)`, `mcp.call("saveState", ...)`, `mcp.call("reloadRecentFile", ...)`, and `mcp.call("setInput", ...)` remain available from callback code. The helper `setCTableSeed` provides the JavaScript equivalent of the common `setCTable_jp.lua` pattern: it writes `0x4b539adb` at `0x02385f0c` and zero at the following word unless overridden.
