...
- `stepOver`: Accepts `{ "cpu": "arm9"|"arm7", "timeoutMs": number, "maxSteps": number }`. ARM9 requires Stack Trace to already be enabled with a recorded active frame. It JS-steps until PC reaches the sequential ARM/Thumb instruction address while execution is back on the same stack lane and the same depth, or stops at `root` if that starting lane becomes shallower or disappears. A matching address reached recursively at a deeper depth or on another lane is not success. Each JS step reapplies Memory Freeze values. Breakpoints and native faults still interrupt safely. Timeout or `maxSteps` returns `complete:false`, `limitReached:true`, and `stop:"timeout"|"maxSteps"` instead of throwing. ARM7 has no native call-stack lane tracking, so ARM7 uses the legacy native step-over implementation and does not inspect ARM9 trace lanes.
...
- `nextCallThisDepth`: ARM9-only lane-aware trace stepper. It binds the starting `activeStackId` and call depth, ignores temporary entry-hook frames that retain the starting frame's return address, ignores depth changes while another lane is active, and stops only when the instruction executed at that starting depth creates a call frame. Calls made after entering a child frame do not satisfy this condition. If the starting lane returns or disappears first, it stops with `complete:false` and `stop:"root"`. Stack Trace must already contain an active frame; timeout and `maxSteps` return an incomplete stopped result.
