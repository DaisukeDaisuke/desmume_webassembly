...
`waitForBreak`, `runUntil`, `runUntilMemoryRead`, `runUntilMemoryWrite`, `runInputSequence`, `recordInput`, `replayInput`, `waitForPause`, `waitForStateLoad`, `waitForFileTransaction`, `waitForScreenChange`, `waitForFrameMatch`, and `waitForScreenStable` share one operation lock. A second long-running operation returns `ok:false` with `error.code="BUSY"`. `getOperation`, `cancelOperation`, `getInputState`, and `releaseInput` remain available for recovery.
