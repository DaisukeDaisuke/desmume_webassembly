...
// emu.registerBaseline(saveCallback, restoreCallback) is the same registration function.
```
Both arguments must be `async` functions. Missing/`null`/non-async callbacks leave the hook unregistered and return `false` without stopping the script; successful registration returns `true`. A save callback may return any bounded JSON-compatible value. `undefined` and `null` are both stored as JSON `null`; the restore callback return value is ignored. Each callback is always serialized with that script's event/MCP work and awaited as a blocking call. If it does not settle within 1500 ms, the baseline operation fails with `TIMEOUT` and that Worker is stopped and discarded so a late result cannot commit state.

`saveAnalysisBaseline` captures registered state from every currently running persistent script before serializing emulator State. Each entry stores the script name, SHA-256 of the exact source, and the callback JSON. `restoreAnalysisBaseline` verifies that every saved entry has a currently running hook with the same name and source SHA-256 before native State is loaded, then restores script JSON while the emulator remains paused and before the recorded run/pause policy is applied. A missing, stopped, renamed, changed, or unregistered script fails restoration with `STATE_INVALID`; scripts started after the baseline that have no saved entry are not modified. A script whose own baseline hook is registered cannot initiate `saveAnalysisBaseline` or `restoreAnalysisBaseline` through its own `mcp.call`, because waiting on itself would deadlock; the call returns `BUSY`.
