...
The current public registry exposes 143 command names. Compatibility aliases are included in this count. `window.DesmumeMCP.list()` is the runtime source of truth; this inventory is the reviewable document counterpart.
...
- `nextCallThisDepth`: ARM9-only lane-aware trace stepper. It binds the starting `activeStackId` and depth, ignores depth changes while another lane is active, and stops at the next direct call on the starting lane. If that lane becomes shallower or disappears first, it stops with `complete:false` and `stop:"root"`. Stack Trace must already contain an active frame; timeout and `maxSteps` return an incomplete stopped result.
