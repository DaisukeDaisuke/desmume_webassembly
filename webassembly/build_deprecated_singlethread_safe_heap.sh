...
# DEPRECATED: This build intentionally disables pthread rasterization so that
# Emscripten ASSERTIONS can provide native crash diagnostics without deadlocking
# the browser main thread. Never use this artifact for performance, threading,
# release, or deployment verification.
printf '%s\n' 'WARNING: deprecated single-thread safe debug build; pthread behavior is not represented.' >&2
...
compile_cpp() {
  ...
  emcc "${INCLUDES[@]}" -O3 -std=c++17 -fexceptions -DWASM_DISABLE_PTHREAD_RASTERIZER -DHAVE_LIBZ -include algorithm -include cassert -sUSE_ZLIB=1 -c "${src}" -o "${obj}"
  OBJECTS+=("${obj}")
}
...
emcc "${OBJECTS[@]}" \
  ...
  -sEXPORTED_FUNCTIONS='["_main","_malloc","_free","_prepareRomBuffer","_loadROM","_reset","_isRomLoaded","_runFrame","_runFrames","_captureFrameBuffer","_fillAudioBuffer","_getSymbol","_setSampleRate","_savGetSize","_savGetPointer","_savUpdateChangeFlag","_savImportFromFile","_savExportToFile","_stateGetSize","_stateGetPointer","_saveStateToBuffer","_loadStateFromBuffer","_loadStateFromFile","_zlibCompress","_zlibDecompress","_pauseEmu","_isPaused","_debuggerSetEnabled","_traceSetEnabled","_traceSetSuspended","_traceSetPrivilegeCheck","_traceGetDepth","_dbgGetReg","_dbgSetReg","_dbgRead8","_dbgRead16","_dbgRead32","_dbgWrite8","_dbgWrite16","_dbgWrite32","_dbgDumpMemory","_dbgSetExecBreakpoint","_dbgSetReadBreakpoint","_dbgSetWriteBreakpoint","_dbgSetSpecialBreakpoint","_dbgClearBreakStatus","_dbgClearAllBreakpoints","_wasmDebuggerShouldBreak","_dbgStep","_dbgStepOver","_dbgGetStatusJson","_dbgDisassemble","_dbgDisassembleOpcode","_dbgStackTrace","_dbgCallStackJson","_dbgCallStackJsonLimit","_chtGetList","_chtAddItem","_utilStrLen","_utilBinaryFloat","_emuSetOpt"]' \
