...
- `nextCallThisDepth` binds ARM9 stack identity and depth together, ignores temporary switches to another lane, and stops at the next direct call or when its starting lane returns; ARM7 is rejected because native trace lanes are ARM9-only. ARM9 trace-off/cold-start calls fail before stepping. Trace steppers report `timeout`/`maxSteps` limits as an honest incomplete stopped result instead of throwing after the bounded JS loop, and the UI logs incomplete stop reasons.
