...
- `callStack` keeps UI-visible `pc-write` rows in the public top-level `frames`, while withholding their synthetic/control-flow bookkeeping fields. This preserves the UI age sequence (`newest`, `↑+1d`, `↑+2d`, ...). Active frames are not duplicated inside `stacks`; that array contains lane summaries only.
...
- `desmume.eval` and persistent scripts use the registered dispatcher. Baseline save/restore requests made while a baseline operation is active return `BUSY`, while ordinary read-only script calls remain available. When an isolated script calls a published handler, the handler result is `response.value`.
...
- `emu_registerbaseline(saveAsync, restoreAsync, priority?)` / `emu.registerBaseline(...)` registers one built-in blocking callback pair per persistent script. Missing or null callbacks do not stop the script; `undefined` save results are stored as JSON `null`. Higher integer priority runs later during restore, so deferred policy effects have deterministic last-wins ordering.
- Analysis baselines enter a global stopped file-transaction boundary, store callback JSON in IndexedDB, and retain only a small localStorage manifest with the script name, priority, and exact-source SHA-256. Restore commits event cancellation before validating one immutable script-instance plan and requires the saved/current participant sets to match exactly.
- Each callback has a 1500 ms deadline measured after callback start. Timeout fails the baseline operation and stops/discards the Worker so a late callback cannot commit. Nested baseline operations return `BUSY`; restore policy changes are deferred until every hook succeeds, and post-native-load failures return `STATE_PARTIALLY_RESTORED` while paused.
