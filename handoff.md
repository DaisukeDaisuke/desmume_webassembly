...
- The current `public/index.html` automation security note is user-owned. It deliberately preserves legitimate debugger/raw-memory/disassembly/Ghidra-style analysis and restricts only external full/near-full ROM/save/state transfer. Do not replace it with an older Codespace copy. Chrome DevTools `evaluate_script` remains a privileged local diagnostic path; page code cannot sandbox that external debugger authority.
