...
- `nextCallThisDepth` binds ARM9 stack identity and depth together when a recorded lane exists, ignores temporary switches to another lane, and stops at the next direct call or when its starting lane returns; ARM7 is rejected because native trace lanes are ARM9-only. If Stack Trace is off, the first invocation enables it and bootstraps from native depth; if tracing was already on but no active lane exists, `stepOver` falls back to bounded native single-stepping with the legacy native sequential-PC target so `timeout`/`maxSteps` and breakpoint stops remain observable, while `nextCallThisDepth` falls back to the existing depth-based next-call path and reports `stop:"call"` on successful fallback completion. Trace steppers report `timeout`/`maxSteps` limits as an honest incomplete stopped result instead of throwing after the bounded JS loop, and the UI logs incomplete stop reasons.
