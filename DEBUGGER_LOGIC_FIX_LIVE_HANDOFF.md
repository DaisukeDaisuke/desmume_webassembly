...
- `public/index.html`'s current `automation-security-context` is a user-owned change. It explicitly preserves legitimate debugger/raw-memory/disassembly/Ghidra-style analysis while prohibiting external full/near-full ROM/save/state dumps, and distinguishes privileged local Chrome DevTools `evaluate_script` from sandboxed WebMCP eval. **Do not revert or overwrite this text from the older Codespace copy.**
