...
- Added a deterministic `.mjs` version with arbitrary fake ROM bytes. It proves opaque origin (`null`) and `https://attacker.invalid` cannot invoke or receive replies for `dumpMemory`, `memoryReadByte`, `memoryReadWord`, `memoryReadDword`, or `getRegisters`; a same-origin control request succeeds so the test is not vacuous. The suite now passes 76/76 locally.
...
## Checkpoint 21 — new merge blocker accepted; previous ready status revoked

- The updated `releaseblocker.md` was read sequentially in 500-line chunks: lines 1-500, then 501-EOF, followed by an empty 1001+ EOF check. No parallel reading was used.
- Checkpoints claiming `81/81` or `ready to submit` are historical only and are not accepted as evidence for the new review. Current status is **merge blocked / implementation in progress**.
- New required work includes: first-party bootstrap before any Acorn/SSIM initializer; ROM/file/native-break/owner identity validation for persistent callback finalization; tracked overflow failure; complete rollback gates; bounded non-breakpoint event queues and stopped-script history; bounded JSON-like Worker results; fail-closed breakpoint reconciliation; bundle-derived license checks; untracked-aware CI artifacts; production-path security self-test; full Chrome DevTools `execute_webmcp_tool` matrix; and native C/C++ contract fixes/failure injection.
- The prior public deployment remains intentionally unchanged. Review-branch Actions must remain build/test-only; Pages upload/deploy is permitted only for a normal push to `main`.

## Checkpoint 22 — new blocker implementation checkpoint (verification not yet complete)

### Completed source changes in this pass

- Removed third-party static imports from `eval.worker.js`, `persistent-script.worker.js`, and `algorithm.worker.js`. Added first-party `dependency-bootstrap.js`; Acorn/SSIM are now built as separate fixed IIFE source blobs with SHA-256 and sent as data only after the Worker has hidden network/storage/sub-Worker/raw-message/crypto/eval/constructor capabilities and verified lockdown invariants. Acorn parses twice inside the locked realm before user compilation.
- Added fixed `security-fixtures/adversarial-dependency.entry.js`, a dedicated hardened production Worker, and argument-free `runSandboxBoundarySelfTest`. The fixture performs no external request and touches no ROM/save/state/memory/register; it reports only forbidden-capability booleans, fixture SHA-256, forgery booleans, and cleanup counts. It is not registered as an algorithm or UI option.
- Added `bounded-value.js`. One-shot eval results are normalized before sandbox-to-supervisor transfer and again at main receipt. Cycles, BigInt, typed arrays, ArrayBuffer, Map/Set, class instances, non-finite numbers, excessive depth/nodes/items/bytes are rejected. Batch normalizes and charges each result before retaining it instead of stringifying the accumulated array afterward.
- Persistent non-breakpoint events now have bounded queues at main, supervisor, and sandbox layers. Tick events coalesce latest-only; lifecycle ordering is retained; queue overflow stops the script with a resource failure. Main/supervisor use event acknowledgements. Added total stopped-script record/history budgets with oldest stopped-record pruning and active-ID repair.
- Persistent breakpoint pending events now save `romGeneration`, `fileTransactionSerial`, native break serial, owner site, event PC/CPU/type/address, pause serial, and callback tokens. Finalization recomputes current ownership and requires the same native break, generation, transaction, PC/site, synchronized breakpoints, no file load, and no explicit pause before step/resume.
- Pending-event saturation no longer log-and-returns. It creates a tracked failed overflow event, pauses, reconciles, stops participant scripts with all-settled cleanup, and exposes typed `BUSY`.
- ROM/save reload now increments a monotonic file transaction and native-break serial, cancels old pending events before filesystem mutation, and marks the transaction active. Rollback checks native return code, waits the boot gate, verifies `isRomLoaded`, then reconciles. Failure records rollback stage/native code/`breakpointsInSync:false` and remains paused.
- Breakpoint reconciliation now calls start/success/failure hooks. The app pauses and sets `breakpointsInSync=false` before native clear/re-register, sets true only after all sites succeed, and clears again on failure. Native resume/frame/step/stepOver refuse execution while out of sync.
- License checking now derives bundled production package names from esbuild metafiles, verifies exact lock versions, an allowlisted lockfile license, and a versioned notice for every bundled package. Hardcoded production package enumeration was removed.
- `build-js.mjs` now generates tracked `public/app.js.sha256`. Actions verifies the tracked notice/hash manifest plus untracked status instead of pretending ignored `public/app.js` can be checked by `git diff`.
- Native fixes applied: `prepareRomBuffer` commits length/capacity only after successful realloc; `loadROM` commits `romLen` after successful native load; paused/no-ROM/completed-frame return semantics were separated; `runFrames` counts only completed frames; debugger dump rejects address+length overflow; pthread compatibility stubs now document lack of functionality and return `ENOTSUP` rather than false success.

### Current verification state and exact resume point

- These new changes are **not yet accepted or ready to submit**. Old 81/81 and old browser evidence remain revoked for this review.
- Codespace `upgraded-xylophone-697q7wgrq5535xpr` was restarted. The complete local `src` directory has just been copied to `/workspaces/desmume_webassembly/`. No new check/build has run yet.
- Next: copy `scripts`, `.github/workflows/webassembly.yml`, `webassembly/wasm-port.cpp`, and `webassembly/support.c`; run `npm run check:js` and `npm run build:js` to expose syntax/bundle issues; copy the generated hash value back locally through `apply_patch`.
- Then add the user-requested deterministic local/Codespace regressions for ROM generation/transaction/owner races, pending overflow, rollback nonzero, reconcile partial failure, slow tick saturation, script history saturation, cyclic/BigInt/typed-array results, malicious dependency initialization, startup race, and native contracts. Run the full suite and safe-heap/sanitize builds as applicable.
- Finally run the real Chrome DevTools MCP `execute_webmcp_tool` matrix, including all four tools and the fixed boundary self-test. Do not use Browser Use. Do not expose real ROM/memory; use synthetic markers only. Review-branch Actions must remain build/test-only and must skip Pages upload/deploy.

## Checkpoint 23 — new merge blockers implemented and locally verified

- The full Codespace suite now passes: **90 tests, 90 passed, 0 failed**. New deterministic coverage is in `tests/merge-blockers.test.mjs` for native-break/file-transaction callback identity, tracked pending-event saturation, rollback failure gates, partial breakpoint-reconcile cleanup, bounded structured values, post-lock adversarial dependency initialization, supervisor event-queue overflow, and native C/C++ source contracts.
- `npm run check:js`, `npm run check:licenses`, `npm run build:notices`, and `npm run build:js` all pass. The generated local `public/app.js` SHA-256 matches tracked `public/app.js.sha256`: `e5153f79905cdce94d7125a2dfd6ea5416eb8a1ba39019f00c71f8c7f4559688`.
- The required `webassembly/build_safe_heap.sh` build succeeded twice. Only pre-existing upstream compiler warnings appeared. A sanitizer build was started because the repository guide recommends it for native memory-boundary work, then stopped immediately when the user clarified that this target is not operationally useful here. Safe-heap was rebuilt afterward and is the final `public/desmume.js` copied locally.
- Chrome DevTools MCP verification used `take_snapshot` before analysis. The fixed production `runSandboxBoundarySelfTest` passed: every pre-ready forbidden capability was false, forgery flags were false, and cleanup counts were zero. `desmume.eval` exposed no DOM/window/network/sub-Worker/raw-message/storage/eval/Function capability; dynamic import failed at compile; a 257-item result failed the item budget; `desmume.runScript` returned a bounded plain result; and self-test parameters were rejected.
- Local ROM verification loaded the user-designated file without reading or returning its contents. Pause remained stable, `stepFrames({frames:3})` completed exactly three frames and restored pause, a uint32-end-crossing dump was rejected, ROM reload succeeded while paused, and a user breakpoint remained registered across reload before being removed as test cleanup.
...
## Checkpoint 25 — 2026-07-22 releaseblocker follow-up implemented

- The new `releaseblocker.md` findings were accepted. Fixed-success self-test attestation was removed; the command now exercises the real eval supervisor/sandbox path and observes forged-message rejection, pending RPC cleanup, Worker termination, Blob URL revocation, and timer cleanup.
- RPC inputs now have shared bounded normalization before the sandbox's first send, supervisor validation, and main validation. Focused tests prove an 8 MiB TypedArray, ArrayBuffer, deep/cyclic values, and oversized arrays produce zero sandbox `call` messages, while ordinary status/memory/disassembly and bounded byte injection remain available.
- Worker prototype-chain lockdown and adversarial fixture coverage now include WorkerGlobalScope/EventTarget methods, constructor aliases, symbols, getters, and raw message handler properties.
- Native full-breakpoint clear includes special flags. Trace runtime data is reset at reset/ROM-load boundaries. Production C++ uses shared runtime-state helpers exercised by the native failure-injection harness.
- `automation-security-context` is 1,344 UTF-8 bytes with every required policy meaning retained.
- Final Codespace results: 98/98 tests, JS/license/notice/bundle checks, and safe-heap WASM build pass. The permanent suite now covers both eval and persistent sandbox first-send rejection, including oversized trigger metadata. Direct Chrome DevTools MCP verification confirmed `desmume.list`, `desmume.call`, `desmume.eval`, `desmume.runScript`, production self-test `passed=true`, all observed cleanup flags, sandbox prototype denial, one-shot and persistent oversized RPC rejection, clean persistent-script stop, no console warning/error, and no external network request.
- Generated local artifacts are `public/app.js` with SHA-256 `bc8ad689be4d8c55c8ecf68bed9cdb11885d9e9ca69570504f562f2754e04c89` and the successful safe-heap `public/desmume.js`. No commit or push was performed in this checkpoint.

## Checkpoint 26 — hostile-intrinsic blocker follow-up in progress

- A newer `releaseblocker.md` (368 lines) was read completely. Previous special-breakpoint, trace reset, native harness, production-path self-test, security note, and basic three-layer RPC fixes were accepted, but release remains blocked because sandbox user code can mutate the intrinsics used by `worker-rpc-value.js` and `bounded-value.js`.
- Required security work now in progress: capture trusted intrinsics before user code; manually copy arrays/bytes into setter-safe objects; use one byte-command schema for `bytes`, `base64`, `hex`, `input`, `text`, `words`, and `opcodes`; prebundle supervisors so they can independently revalidate every child outbound message; add explicit inner shutdown acknowledgement and separate inner/outer cleanup measurements; add bundled intrinsic-mutation regressions.
- Implemented so far in this checkpoint: added `src/trusted-value-normalizer.js`; routed `bounded-value.js` through it; replaced `worker-rpc-value.js` with shared command schemas and trusted normalization; started decoded-size enforcement in `binary-tools.js`, `memory-commands.js`, and `disassembly-commands.js`. These changes have not yet been run in Codespace and must not be treated as verified.
- The later `screen-shell` report was explicitly withdrawn. Do not change rotation, canvas, stage, or shell layout for this checkpoint.
- Exact continuation: finish supervisor prebundling/outbound validation and self-test shutdown protocol; add intrinsic/alias/cleanup regressions; then sync every changed file to Codespace, run the full required checks and safe-heap build, copy generated artifacts, and verify the security paths with Chrome DevTools MCP only. Stop the Codespace at completion.

## Checkpoint 27 — hostile-intrinsic blocker implemented and Codespace-verified

- Added a module-initialized trusted normalizer that captures the required Array/Object/Number/TextEncoder/TypedArray/Set/Reflect intrinsics before user code. It uses captured descriptor access, dense manual array copying, manual byte copying, setter-safe property definition, and null-prototype structured objects. Mutating `TextEncoder.prototype.encode`, Array helpers, Object static helpers, Number predicates, or `Uint8Array.prototype.slice` no longer bypasses the first-send limits.
- Eval and persistent supervisors are now prebundled with the same boundary modules. They reconstruct and independently validate every authenticated child output, including RPC params, triggers, eval results, print values, failures, and event completions; no child object is forwarded by spread. Oversized `done`/`print` and malformed output terminate the child and fail closed.
- `injectBytes` and `disassembleBytes` share one command schema for `bytes`, `base64`, `hex`, `input`, `text`, `words`, and `opcodes`. Both encoded prechecks and decoded byte/word limits are enforced in the command parser. Final WebMCP `content` and `structuredContent` are independently bounded before publication.
- Supervisors accept an explicit `shutdown` request and return `shutdownAck` only after attempting real child handler clearing, Worker termination, Blob URL revocation, pending-RPC clearing, and (for persistent scripts) event-queue clearing. The production boundary self-test separately reports observed inner and outer cleanup. Security probes pass only when `code=SECURITY_PROBE_REJECTED`, `phase=child-auth`, and the exact `probeId` all match.
- Permanent regressions cover intrinsic mutation, zero-call oversized RPC rejection, bounded typed-byte copying, oversized eval results, persistent print rejection, direct supervisor oversized output, every byte alias, exact security-probe identity, and shutdown cleanup fields. Codespace results: **102/102 tests passed**; `check:js`, `check:licenses`, `build:notices`, `build:js`, and the safe-heap WASM build succeeded. Generated `public/app.js`, `public/app.js.sha256` (`eaee7026e8d201c7d0125faee75a34085e8dd4163ffb6a800e6f254f7f5e3d70`), `public/desmume.js`, and notices were copied locally.
- Direct Chrome DevTools MCP verification could not start: after DevTools reconnected and exposed `about:blank`, the first navigation to `http://localhost:8766/` was rejected by the tool provider's usage limit. No Browser Use or alternate browser workaround was attempted. Browser verification therefore remains the only uncompleted acceptance item.
- The withdrawn `screen-shell` report was not acted on. No rotation, canvas, stage, or shell-layout source was changed in this checkpoint.

## Checkpoint 28 — pre-first-postMessage limit/schema/error blockers fixed

- The latest `releaseblocker.md` was read and treated as the active release blocker. Existing supervisor/sandbox split, independent supervisor revalidation, RPC allowlists, cleanup accounting, and previous deterministic coverage were preserved.
- Fixed normalizer option handling in `src/trusted-value-normalizer.js`: limit reads now use captured own data descriptors only, ignore inherited `Object.prototype` pollution, reject accessors by omission, validate finite non-negative safe integer limits, avoid `maxArray` fallback for `maxProperties`, validate nested `stringLimits` and `specialArrays.maxItems`, and keep absolute trusted maxima.
- Fixed command schema lookup in `src/worker-rpc-value.js`: byte-command schemas are now stored in a `Map`, so `__proto__`, `constructor`, `toString`, etc. cannot resolve prototypes as schemas. Options passed into the trusted normalizer are copied from known own data fields only, without spreading untrusted schemas.
- Added `src/worker-error-serializer.js` and routed eval/persistent sandbox failures through it before the first child `postMessage`. It truncates by UTF-8 byte count, avoids normal property access and user-defined `toString()`, does not execute getters, bounds stack/source excerpt fields, and fail-closes to a fixed small error if serialization itself fails. Native `SyntaxError`/`TypeError` names are recovered only from captured native error prototypes.
- Preserved byte-command compatibility at the Worker boundary for string array elements such as `{ bytes: ["0x01", "ff"] }`, `{ words: ["0x12345678"] }`, and `{ opcodes: ["0xe12fff1e"] }` while keeping decoded array size limits.
- Added focused regressions to `tests/release-blockers.test.mjs` for inherited polluted limits, unsafe explicit limits, nested schema limit validation, prototype command names, eval huge return after prototype pollution, eval `__proto__` RPC after prototype pollution, bounded eval error fields, getter-safe eval error serialization, bounded persistent failure fields, getter-safe persistent failure serialization, and string byte/word array compatibility.
- Local dependency-free checks passed: `node --check` for touched source/test files, direct Node ESM reproduction for `Object.prototype.maxBytes = Infinity`, targeted `git diff --check` for the touched files, and `node --test tests/core-services.test.mjs tests/refactor-services.test.mjs` with 44/44 pass. Local `release-blockers`/`merge-blockers` were not run because local `esbuild` is missing and dependencies must not be installed locally.
- Codespace `upgraded-xylophone-697q7wgrq5535xpr` was started with SSH, then `src/` and `tests/` were transferred by directory. Codespace `npm test` passed **107/107**. A direct explicit test command for `core-services`, `refactor-services`, `merge-blockers`, and `release-blockers` also passed **107/107** after fixing native `SyntaxError` name preservation.
- Codespace `npm run check:js`, `npm run check:licenses`, `npm run build:notices`, and `npm run build:js` completed successfully. The final chained command failed only at unrestricted `git diff --check` because the Codespace working tree has broad old/trailing-whitespace diffs in files outside this patch; targeted local `git diff --check` for touched files passed.
- A `public/` folder copy from Codespace was attempted before the user clarified that the Codespace tree is old. It only changed `public/app.js.sha256`; that accidental change was reverted with `apply_patch`. Do not treat Codespace-generated public artifacts from this checkpoint as current release artifacts.
- Chrome DevTools MCP acceptance has not been run in this checkpoint. Browser Use was not used.
