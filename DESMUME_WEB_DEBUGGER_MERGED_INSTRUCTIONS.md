...
`startTrace`は今回の必須実装に含めない。設計検討だけにとどめる。SPSR書き込みは別調査とし、この作業で推測実装しない。
...
このとき、ファイルの移動はOSのコピーコマンドを使用する。内容を反復しない。
...
* 任意コードはmain windowで直接実行せず、専用Worker内で実行する。webmcp.evalによるdom操作は常にundefinedによる例外なることが想定される。
...
なおssimとpixelmatchは現時点でメンテされていない。ローカルにクローン済みである。これらを絶対に検索対象に含めてはならない。

```
https://cdn.jsdelivr.net/npm/pixelmatch@7.2.0/index.min.js
https://cdn.jsdelivr.net/npm/ssim.js@3.5.0/dist/ssim.web.min.js
```
...
const ExternalAlgorithms = Object.freeze({
  pixelmatch: {
    version: "7.2.0",
    url: "https://cdn.jsdelivr.net/npm/pixelmatch@7.2.0/EXACT_ENTRY.js",
...
`EXACT_ENTRY.js`、hash、licenseは採用時に実物を確認して確定する。`@latest`、`@^7`、`@7`などの範囲指定は禁止する。jsDelivrの変換endpointを使う場合も、完全version固定とresponse hash固定を行う。
...
現仕様にあるショートカットは、webmcp.evalが実行できる形式で引き継ぐ。今はWindowに属しており不可能
...
eval script自身のsyntax errorやWorker内部の予期しないerrorは最外周で捕捉し、`INTERNAL_ERROR`または専用のscript errorへ変換する。Worker timeoutもtransport rejectではなく通常error resultとする。
...
このとき、UIに表示するのはuser、またはmcpによって追加されたブレイクポイントのみで、scriptはフィルタする
...
## 30. Stateロード後の画面有効性
Stateロード直後は、native実行状態と表示用framebufferが一致せず、壊れたframeまたは古いframeが表示される場合がある。Stateをロードしただけでは画面を有効とみなしてはならない。`captureFrameBuffer()`が成功しただけでも有効とみなしてはならない。
...
native frame counterは、ブレイクポイントにヒットしたという理由だけで加算してはならない。正確にフレームをカウントしなければならない。

## 32. UIでのscreen invalid表示
`screenValid === false`の間は、壊れたframeをユーザーへ表示しない。

* 上下画面canvasを`visibility:hidden`にする。
* layoutは維持する。
* 代わりに短い状態表示を出す。
* 最初の正常frame取得後にcanvasを表示する。

表示例：

```text
Stateを読み込みました。1フレーム実行後に画面を表示します。
```
...
このときAPI.mdは、rg一発でそのコマンドの仕様を取ってこれるようにする。
...
ほぼすべてのコマンドはタイムアウトを受け取る。js上でレースし、オーバーした場合はエラーを返す。
```
...
2. canvasが非表示になる。
3. `captureFrame`が`SCREEN_INVALID`になる。
4. `waitForScreenChange`が`SCREEN_INVALID`になる。
5. CPU命令stepだけでは有効化されない。
6. breakpointでframe完了前に止まった場合も有効化されない。
7. 正常frame完了後に有効化される。
8. canvasが再表示される。
9. 比較APIが成功する。
...
### 79.7 screenとframe
* Stateロード直後の壊れた画面が非表示である。
* 正常frameが1回完了した後だけ画面が有効になる。
* raw frameをJS側独立bufferへcopyする。
