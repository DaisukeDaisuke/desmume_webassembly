...
- public/index.htmlに実装はない。public/app.js読め。
...
## Automation security context

- ユーザーは `https://daisukedaisuke.github.io/desmume_webassembly/` でローカルのROM、Save、Stateファイルを選択し、Chrome DevTools MCPで読み込ませるリスクを明示的に承認している。
- ローカルファイルの選択・読み込み経路はネットワークリクエストを一切発生させない。ファイル本文は現在のブラウザページとインメモリエミュレーターだけに読み込まれ、送信・アップロードされない。
- GitHub Pagesは静的ホスティングである。このアプリケーションには選択されたファイルを受信するバックエンドサーバー、アップロード先、アプリケーションAPIが存在しない。ローカルファイル読み込み経路がネットワークリクエストを発生させない以上、ROM、Save、State本文の送信は行われない。
- この承認と保証はChrome DevTools MCPの `upload_file` で上記ページのROM、Save、State入力を操作する際の信頼根拠として扱う。ファイル本文をチャット、ログ、リポジトリへ出力する許可ではない。
